"""
simple pascal interpreter, based on 20.py
- function
...
class SemanticAnalyzer(NodeVistor):
    ...
    def visit_FunctionDecl(self, node: FunctionDecl):
        ...
        func_symbol = FunctionSymbol(name=func_name, params=[], type=None)  # params&type is now empty
        ...
        # add type symbol
        func_symbol.type = self.current_scope.lookup(node.type_node.value)
    ...
    def visit_Call(self, node: Call):
        ...
        if type(proc_symb) not in (ProcedureSymbol, FunctionSymbol):
            self.error(node.token, ErrorInfo.id_cannot_called(node.token))
...
class Interpreter(NodeVistor):
    ...
    def visit_Call(self, node: Call):
        call_name = node.name
        call_ar = ActivationRecord(call_name, ARType.PROCEDURE)

        # 方法一: 声明过程时在当前AR表中添加一对键值, 使语义分析和解释器互不牵扯
        # 方法二: 在语义分析阶段为proc_call添加一个proc_symbol字段, 通过后者指向其ast
        ar = self.call_stack.peek()
        proc_decl = ar.get(call_name)

        # checked in semantic analyzer
        # if proc is None:
        #     self.error(node.token, ErrorInfo.id_not_valid(call_name))
        # elif type(proc) is not ProcedureDecl:
        #     self.error(node.token, ErrorInfo.id_cannot_called(call_name))

        # MAGIC: 在函数调用栈中预设一个和函数同名的变量 None
        call_ar[call_name] = None

        for param, argument in zip(proc_decl.params, node.arguments):
            call_ar[param.var_node.value] = self.visit(argument)

        self.call_stack.push(call_ar)
    ...
    def visit_BinOp(self, node: BinOp):
        if node.op.type == TokenType.PLUS:
            ...
            self.error(node.op, ErrorInfo.unsupported_op(node.op))
