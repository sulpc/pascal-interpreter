'''
simple pascal interpreter, based on 11.py
...
class Interpreter(NodeVistor):
    ...
    def pack_Program(self, node: Program):
        data = {
            'name': f'{node.name}',
            'children': [self.pack(node.block)]
        }

        return data

    def pack_Block(self, node: Block):
        decls = {
            'name': 'Decls',
            'children': []
        }
        for decl in node.declarations:
            decls['children'].append(self.pack(decl))

        data = {
            'name': 'Block',
            'children': [decls, self.pack(node.compound_statement)]
        }
        return data

    def pack_ProcedureDecl(self, node: ProcedureDecl):
        data = {
            'name': f'Proc-{node.proc_name}',
            'children': [self.pack(node.block_node)]
        }
        return data

    def pack_VarDecl(self, node: VarDecl):
        data = {
            'name': 'VarDecl',
            'children': [self.pack(node.var_node), self.pack(node.type_node)]
        }
        return data

    def pack_Type(self, node: Type):
        data = {
            'name': f'{node.value}',
        }
        return data

    def pack_Compound(self, node: Compound):
        data = {
            'name': 'Compound',
            'children': []
        }
        for stat in node.children:
            data['children'].append(self.pack(stat))
        return data

    def pack_Assign(self, node: Assign):
        data = {
            'name': ':=',
            'children': []
        }
        data['children'].append(self.pack(node.left))
        data['children'].append(self.pack(node.right))
        return data

    def pack_NoOp(self, node: NoOp):
        data = {
            'name': 'NoOp',
        }
        return data
